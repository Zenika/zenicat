...
function App() {
  ...
  const messagesFromSlack = useCallback(async () => {
    ...
    dispatch({
      ...
      messages: (await mSlack.json()).messages
    });
  }, [dispatch, thread]);
    ...
    <div className="App">
      ...
      <div className="container">
        {messages.map(message => (
          <div
            className={message.from === "me" ? "content-right" : "content-left"}
          >
            <p
              key={message.text}
              className={message.from === "me" ? "right" : ""}
            >
              {message.text}
            </p>
          </div>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            const response = await fetch(
              `/question?text=${input}${
                localStorage.slack || thread
                  ? `&thread_ts=${localStorage.slack || thread}`
                  : ""
              }&username=${inputFirstName}&uuid=${localStorage.uuid}`
            );
            if (!thread) {
